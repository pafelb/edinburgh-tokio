public class gamestate {
    ...
    private void updateFleet() {
        int pointer = 998;
        int current_player_move = game.getCurrent_player_move();
        int[] split = game.getSplitLocos();
        for (int i = 0; i != 4; i++) {
            int actualPlayer = (current_player_move + i) % 4;
            inVec[pointer++] = (60 - split[actualPlayer]) / 50f;
            inVec[pointer++] = split[actualPlayer] / 25f;
        }
    }

    private void updateTotalBuildt() {
        int pointer = 1016;
        int current_player_move = game.getCurrent_player_move();
        int[][] buildt = game.getBoats_locos_buildt();
        for (int i = 0; i != 4; i++) {
            int actualPlayer = (current_player_move + i) % 4;
            inVec[pointer++] = buildt[actualPlayer][0] / 50f;
            inVec[pointer++] = buildt[actualPlayer][1] / 25f;
        }
        for (int i = 0; i != 4; i++) {
            int actualPlayer = (current_player_move + i) % 4;
            inVec[pointer++] = (buildt[actualPlayer][0] + buildt[actualPlayer][1]) / 60f;
        }
    }
    private void updateHarborsAm(){
        int pointer = 1014;
        int current_player_move = game.getCurrent_player_move();
        int[] buildt = game.getHarborsBuildt();
        for (int i = 0;i!=4;i++) {
            int actualPlayer = (current_player_move + i) % 4;
            inVec[pointer++]=buildt[actualPlayer]/3f;
        }
    }
    private void updateTicketCount(){
        int pointer = 1018;
        int current_player_move = game.getCurrent_player_move();
        int[] ticketAm = game.getTicketcount();
        for (int i = 0;i!=4;i++) {
            int actualPlayer = (current_player_move + i) % 4;
            inVec[pointer++]=ticketAm[actualPlayer]/65f;
        }
    }
    private void drawCardInfo(){
        int pointer = 1022;
        inVec[pointer]= game.isFirstCardDraw() ? 1.0f : 0.0f;
    }
...
//787-823 harbor op2
//824-860 harbors op3
//861-960 knownCards         scaled by total of each
//961-968 unkownCards       scaled by total of boats lokos
//969-970 discard       stacks scaled by total of boats and lokos
//971-972 random stacks      scaled by total of boats and lokos
//973-997 known used discarded cards         scaled by total of boats and lokos
//998-1005 fleet comp, scaled by max ammount possible
//1006-1013 build boats and lokos, scaled by max am each
//1014-1017 harbors,scaled by max ammount
//1018-1021 ticket ammount scaled by max
//1022 info about first or second card draw
//1023-1047 open cards, scaled to max ammount in game
